...
Fermi::contract3210(mdarray3 const& A, mdarray2 const& B) -> mdarray3
{
    ...
    // A[c,e,d]
    // B[s,c]
    assert(263 == B.extent(0));
    assert(A.extent(0) == B.extent(1));
    assert(A.extent(0) == 40);
    assert(A.extent(1) == 38);
    assert(A.extent(2) == 401);
    ...
    // R[s,e,d]
    auto rv          = vector<double>(Ns * Ne * Nd, 0.0);

    // auto       sR    = mdspan3(rv.data(), Ns, Ne, Nd);
    // auto const sA    = mdspan(A.container().data(), Nc, Ne, Nd);
    // auto const sB    = mdspan(B.container().data(), Ns, Nc);
    // co_contract3210(sR, sA, sB);

    Eigen::Map<const Eigen::MatrixXd> mA(A.data(), Nc, Ne * Nd);
    Eigen::Map<const Eigen::MatrixXd> mB(B.data(), Ns, Nc);
    Eigen::Map<Eigen::MatrixXd>       mC(rv.data(), Ns, Ne * Nd);
    mC.noalias() += mB * mA;
...
Fermi::mul32_1(mdarray3 const& A, mdarray2 const& B) -> mdarray3
{
    ...
    size_t const& Ni = A.extent(0); // S
    size_t const& Nj = A.extent(1); // E
    size_t const& Nk = A.extent(2); // D
    ...
    for (size_t i = 0; i < Ni; ++i)
    {
        for (size_t j = 0; j < Nj; ++j)
        {
            for (size_t k = 0; k < Nk; ++k) { R(i, j, k) = A(i, j, k) * B(i, j); }
        }
    }
