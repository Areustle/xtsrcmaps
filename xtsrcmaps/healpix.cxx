    ...
    -> uint64_t
{
    ...
    uint64_t const order_   = nside2order(nside_);
    {
        if (za <= twothird) // Equatorial region
        {
            ...
            uint64_t jp    = uint64_t(temp1 - temp2); // index of  ascending edge line
            uint64_t jm    = uint64_t(temp1 + temp2); // index of descending edge line
        ...
        else /* polar region, za > 2/3 */
        {
            ...
            jp = std::min(jp, nside_ - 1); // for points too close to the boundary
            jm = std::min(jm, nside_ - 1);
            return (z >= 0) ? xyf2nest(nside_ - jm - 1, nside_ - jp - 1, ntt, order_)
