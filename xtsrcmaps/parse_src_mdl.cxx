// An implementation converting an XML file into a vector of Fermi::Source objects.
...
// Return a vector of Fermi::SourceParameters by iterating through the 'parameter' child
// nodes of the given RapidXML node .
auto
...
parse_spatial_model_node(RXNPtr spamo) -> Fermi::SpatialModel
{
    ...
    auto spatial_model_parser
        = [=](std::string const& s) -> std::function<Fermi::SpatialModel(RXNPtr)> {
...
parse_spectrum_node(RXNPtr spec) -> Fermi::Spectrum
{
    ...
    if (type == "FileFunction")
    {
        return Fermi::FileSpectrum {
            ...
            src_par_vec(spec),
            std::string(spec->first_attribute("apply_edisp")->value()) == "true",
        };
    }
    return Fermi::NormalSpectrum {
        ...
        [](std::string const& s) -> Fermi::SpectrumType {
            ...
            if (s == "BrokenPowerLaw") return Fermi::SpectrumType::BrokenPowerLaw;
            if (s == "BrokenPowerLaw2") return Fermi::SpectrumType::BrokenPowerLaw2;
            ...
            if (s == "PLSuperExpCutoff2") return Fermi::SpectrumType::PLSuperExpCutoff2;
            if (s == "PLSuperExpCutoff3") return Fermi::SpectrumType::PLSuperExpCutoff3;
            if (s == "PLSuperExpCutoff4") return Fermi::SpectrumType::PLSuperExpCutoff4;
...
// Return a Fermi::Source given a RapidXML node .
auto
Fermi::parse_src_xml(std::string const& src_file_name) -> std::vector<Fermi::Source>
{
    auto result          = std::vector<Fermi::Source>();
    auto doc             = rapidxml::xml_document<>();
    auto src_file_stream = std::ifstream(src_file_name);
    auto src_file_buf    = std::vector(std::istreambuf_iterator { src_file_stream },
                                    std::istreambuf_iterator<char>());
    src_file_buf.push_back('\0');

    // auto src_sstream     = std::stringstream();
    // src_sstream << src_file_stream.rdbuf();
    // src_file_stream.close();
    // auto src_file_buf = std::string(src_sstream.str());

    doc.parse<0>(&src_file_buf[0]);

    // The source_library root node must be a sequence of sources, which we shall
    // parse
