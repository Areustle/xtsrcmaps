...
auto
Fermi::ModelMap::integral(Tensor<double, 1> const& angles,
    ...
    long const Ne          = mean_psf.extent(2);
    long const Ns          = mean_psf.extent(0);

    PSF::SepArr const seps = PSF::separations();

    assert(angles.extent(0) == Ns);

    Tensor<int, 1> const sep_idxs = Fermi::PSF::fast_separation_lower_index(angles);
